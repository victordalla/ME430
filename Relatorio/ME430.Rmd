...
A amostra piloto confirmou a conjectura. Na Tabela \ref{tab:piloto-media}, observa-se que a pontuação foi maior em estratos de maior renda. A amostra piloto também resultou em uma estimativa de $\hat{s}_{es}^2 = `r mu_s2hat_piloto`$. De posse dessa informação, uma amostragem aleatória simples sem reposição (AASs) com precisão $\delta = `r mu_delta`$ requer $n \geq \Big( \frac{\delta^2}{\hat{s}_{es}^2 z_{\gamma}} + \frac{1}{N} \Big)^{-1}$ unidades amostrais para garantir que $P(|\mu - \hat{\mu}_{es}| \leq \delta) \geq `r mu_gamma`$ \cite{bolfarine2005elementos}. Para os dados dessa amostra, $n = `r mu_n_ASSs`$.

Por outro lado, para realizar AE com AP, os mesmos estratos definidos anteriormente e erro de estimativa $\delta = `r mu_delta`$, é preciso um tamanho de amostra $n \geq \Big(\frac{\delta^2}{z_{\gamma} \sum_{h=1}^H {W_h\hat{s}_h^2}} + \frac{1}{N}\Big)^{-1}$ para garantir que $P(|\mu - \hat{\mu}_{es}| \leq \delta) \geq `r mu_gamma`$ \cite{bolfarine2005elementos}. Para os dados observados, $n = `r mu_n_AE_AP`$. Como esse plano amostral requer menos unidades amostrais para obter o mesmo desempenho, é mais vantajoso AE com AP e a estratificação definida acima a AASs.
...
A amostragem final resultou em $\hat{\mu}_{es} = `r mu_hat`$ para a pontuação média dos candidatos, com variância aproximada de $\hat{Var}(\hat{\mu}_{es}) = `r mu_varhat_muhat`$ e intervalo de confiança de $[`r mu_ic[1]`;`r mu_ic[2]`]$ a um nível de confiança de $95\%$. Os resultados por estrato estão disponíveis na Tabela \ref{tab:inf-media}.
...
A amostragem final resultou em $\hat{p}_{es} = `r prop_phat`$ para a proporção de candidatos que estudaram completamente em escolas públicas no ensino médio, com variância aproximada de $\hat{Var}(\hat{p}_{es}) = `r prop_varhat_phat`$ e intervalo de confiança de $[`r prop_ic[1]`;`r prop_ic[2]`]$ a um nível de confiança de $95\%$. Os resultados por estrato estão disponíveis na Tabela \ref{tab:inf-prop}.
