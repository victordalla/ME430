...
A amostra piloto resultou em uma estimativa de $\hat{\sigma_{\mu}^2} = \hat{\sigma_d^2} + \hat{\sigma_e^2} = `r mu_s2hat_d_piloto + mu_s2hat_e_piloto`$ **USAMOS ASSs DEVEMOS UTILIZAR ESTIMADOR NAO VICIADO S^2, NAO SIGMA^2** **(HELP WANTED)** para a variância $\sigma_{\mu}^2$, onde $\hat{\sigma_d^2} = \sum_{i=1}^H \frac{N_h}{N}\hat{\sigma_h^2}$ é a variância estimada no h-ésimo estrato e $\hat{\sigma_e^2} = \sum_{i=1}^H \frac{N_h}{N}(\hat{\mu}_h-\mu)^2$ é a variância estimada das médias dos estratos. 
De posse dessa informação, para realizar AE com AP e erro de estimativa $\delta = `r mu_delta`$, é preciso um tamanho de amostra $n$ de pelo menos $n \geq \frac{z_{`r mu_gamma`}^2}{\delta^2} \sum_{i=1}^H\frac{N_h}{N} \hat{\sigma^2}$ **(OU SERIA S^2 AQUI?)** para garantir que $P(|\mu - \hat{\mu}| \leq \delta) \geq `r mu_gamma`$ \cite{bolfarine2005elementos}, onde $z_{`r mu_gamma`}$ é o `r mu_gamma`-quantil da normal padrão. Portanto, para os dados coletados, são necessárias pelo menos `r mu_n` unidades amostrais.

Por outro lado, uma amostragem aleatória simples sem reposição (AASs) requer $\Big(\frac{\delta^2}{\hat{s^2}z_{`r mu_gamma`}} + \frac{1}{N}\Big)^{-1}$ unidades amostrais para garantir que $P(|\mu - \hat{\mu}| \leq \delta) \geq `r mu_gamma`$ \cite{bolfarine2005elementos}, onde $s_{\mu}^2 = \frac{n\sigma^2}{n-1}$. Para os dados observados, esse plano amostral precisa de `r mu_n_ASSs` unidades amostrais, número maior que no caso AE com AP. Portanto, é mais vantajoso realizar AE com AP. 
...
\hline
1 & `r mu_N_h[1]` & `r mu_estat_piloto$s2hat_h[1]` & `r mu_estat_piloto$n_h[1]` \\
2 & `r mu_N_h[2]` & `r mu_estat_piloto$s2hat_h[2]` & `r mu_estat_piloto$n_h[2]` \\
3 & `r mu_N_h[3]` & `r mu_estat_piloto$s2hat_h[3]` & `r mu_estat_piloto$n_h[3]` \\
4 & `r mu_N_h[4]` & `r mu_estat_piloto$s2hat_h[4]` & `r mu_estat_piloto$n_h[4]` \\
\end{tabular}
\end{table}
...
1 & `r prop_N_h[1]` & `r prop_phat_h_piloto[1]` & `r prop_s2hat_h_piloto[1]` \\
2 & `r prop_N_h[2]` & `r prop_phat_h_piloto[2]` & `r prop_s2hat_h_piloto[2]` \\
\end{tabular}
\end{table}
