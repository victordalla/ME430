...
\begin{table}[!h]
\centering
\caption{Comparação das estimativas com valores reais. *Parâmetro* é o parâmetro populacional, *Estimativa* é a sua estimativa obtida, *Valor verdadeiro* é seu valor verdadeiro. *Precisão usada* é o $\delta$ respectivo para cada parâmetro determinado na seção Análise Descritiva \ref{s.descritiva} e *Erro absoluto* é calculado como $|\hat{\theta} - \theta|$, onde $\hat{\theta}$ é a estimativa do parâmetro $\theta$.}
...
tau & `r format(tau_hat, digits=4)` & `r format(tau, digits=4)` & `r format(tau_delta, digits=4)` & `r format(erro_abs_tau, digits=4)` \\
