...
A Seção \ref{s.descritiva} contém nossas análises sobre os dados, tanto nas informações que conhecemos no nível populacional, quanto nas obtidas por meio de amostras. Em seguida na Seção \ref{s.inferencia} apresentamos nossas análises, estimadores pontuais das estatísticas de interesse e seus intervalos de confiança. As seções são dividas em subseções para cada estimador, o quais chamamos de, *Média*, *Proporção* e *Total*, respectivamente aos itens i), ii) e iii) da tarefa.

# Análise Descritiva
\label{s.descritiva}

Tratamos as análises realizas abaixos como indepentes visto que o ambito de cada análise são diferentes, sendo assim, similar ao caso de cada estimação ser realizada por uma empresa diferente.
...
Sabemos que no Brasil o ensino fundamental público possui qualidade inferior que nos setores privados, o mesmo pode ser dito para o ensino médio com exceção de colégios técnicos, militares ou a escolas relacionas a universidades públicas. Tal que, na maior parte da população os alunos frequentam o ensino público por necessidade e não opção. Desta forma, é incomum um aluno alterar do setor público para o privado e vice-versa (FINALIZAR...)

Assim no nível populacional observamos que existe uma correlação de `r cor_fund1_fund2` entre o setor da instuição de um indíviduo em um nível (Ensino Fundamental 1, Ensino Fundalmental 2) ao nível subsequente.

Com essa informações podemos observar que alunos que estudaram o Ensino Fundamental 1 completamente em instituições públicas apresentam frequentaram o Ensino Fundamental 2 público numa proporção diferente do restante dos indivíduos.
...
Observamos no nosso banco de dados que temos informações sobre os utensílios domésticos, cômodos das casas e da estrutura familiar dos inscritos no vestibular da COMVEST, como estamos interessados em estimar o total de quartos de uma casa, iremos observar se existe alguma ligação entre a quantidade de banheiros, pessoas da famílias dependentes da mesma renda familiar e quantidade de televisões na casa, visto que normalmente uma casa com muitos quartos possue mais banheiros, uma família com mais dependente dada uma renda familiar pode conter mais quartos e televisões usualmente ficam em cômodos diferentes de uma casa.
...
(sd(smp$Q15) / mean(smp$Q15)) / (sd(smp$Q32C)/mean(smp$Q32C))

# Deve ser menor que rho(x, y)
(sd(smp$Q15) / mean(smp$Q15)) / (2 * sd(smp$Q32C)/mean(smp$Q32C))
...
```{r tamanho-media}
delta <- 1
z_gamma <- qnorm(0.975) # ALTEREI para 0.975 para um intervalo de 0.95 de área
...
Conforme observado na Subseção \ref{ss.descr.prop} os alunos tem o hábito de se manter no mesmo setor do ensino ao avançar nos níveis de educação, antes de entrar no ensino superior, assim dividiremos nossa população em dois estratos, participantes que cursaram o Ensino Fundamental 1 por completo em escolas públicas, o qual chamaremos de estrato 1 e o estrato 2 com os indivíduos restantes.
...
De acordo com essa divisão retiramos uma amostra piloto de tamanho `r n_pil_final`, onde cada estrato foi amostrado com um tamanho proporcional a sua população e verificamos se o comportamento de se manter no mesmo setor do ensino quando se passa do Ensino Fundamental 2 para o Ensino Médio é similar ao do observado na Subseção \ref{ss.descr.prop}.

Na Tabela \ref{tab:piloto-prop} apresentamos as estatísticas da nossa amostra piloto.
Utilizando essas informações vemos que a variância dentro dos estratos, $\hat{s_d^2}= \ $ `r s2hat_d` é consideravelmente menor que a estimativa da variância da amostra, $\hat{s^2} = \hat{s_d^2} + \hat{s_d^2} = \ $ `r s2hat_d + s2hat_e`.
...
Com as estatísticas obtidas da amostra piloto estabelecemos nossa margem de erro desejada, $\delta = \ $ `r delta`, e nosso intervalo de confiança desejado, $\gamma = 0.95$, assim conforme a Equação \ref{eq:n-prop-estrat} apresentada no Apêndice, obtemos o tamanho amostral $n = \ $ `r n`. 
...
Adicionar comentários sobre está formula (aloc otima de neyney)
