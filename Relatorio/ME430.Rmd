...
```{r piloto-media}
# estratos
N_h <- group_by(dados_media, Q14) %>% 
  summarise(N_h = n()) %>% .$N_h
W_h <- N_h / N
H <- length(N_h)
estratos <- select(dados_media, Q14) %>% distinct()

# amostra piloto sob alocação propocional
n_piloto <- 200

piloto_media <- do.call(
  rbind,
  lapply(
    estratos$Q14, 
    function(h) {
      sample_n(select(dados_media, EMPCT, TOTAL, Q14) %>% filter(Q14 == estratos$Q14[h]), 
               size = ceiling(n_piloto * W_h[h]), replace = F)
})
)

stats <- group_by(piloto_media, Q14) %>% 
  summarise(n_h = n(), 
            mean_h = mean(TOTAL), 
            var_h = var(TOTAL), 
            sd_h = sd(TOTAL))
...
tau_amostra_piloto <- sample_n(questionario, n_piloto)
tau_f_piloto <- tau_amostra_piloto/N

tau_hat_piloto <- N * mean(tau_amostra_piloto$Q32C, na.rm = TRUE)
tau_s2hat_piloto <- var(tau_amostra_piloto$Q32C, na.rm = TRUE)
```

```{r total-tam-amostral}
tau_delta = 3500
tau_gamma <- 0.95
tau_z_t <- qnorm(tau_gamma)

# tau_n <- ceiling(1 / ((tau_delta^2/(N^2*tau_s2hat_piloto^2*tau_z_t^2)) + 1/N))
tau_n <- 1 / ((tau_delta^2/(N^2*tau_s2hat_piloto^2*tau_z_t^2)) + 1/N)
tau_n <- ceiling(tau_n * (1 + sum(is.na(tau_amostra_piloto$Q32C)) / nrow(tau_amostra_piloto)))
```
...
s2hat_e <- sum(prop_W_h * (prop_phat_h - prop_phat)^2)
s2hat_d <- sum(prop_W_h * prop_phat)
```
...
tau_hat <- N * mean(questionario$Q32C, na.rm = TRUE)
tau_s2hat <- var(questionario$Q32C, na.rm = TRUE)
tau_varhat <- N^2 * (1 - tau_f) * tau_s2hat / tau_n
```
...
\begin{tabular}{lllll}
$N$ & $n$ & $\hat \mu$ & $\hat{Var(\hat \mu)}$ & $\hat{s^2}$ \\
\hline
`r N` & `r N` & `r format(tau_hat, digits=4)` & `r format(tau_varhat, digits=4)` & `r tau_s2hat`
\end{tabular}
\end{table}
...
$$ n = \Bigg\lceil\frac{1}{\frac{\delta^2}{z_{\gamma} \sum_{h=1}^H {W_h\hat{s_h^2}}} + \frac{1}{N}}\Bigg\rceil $$
