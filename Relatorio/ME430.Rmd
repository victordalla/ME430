...
```{r prop-piloto}
n_piloto <- 200
piloto_prop <- sample_n(questionario, n_piloto)
...
gamma <- 0.95
z_gamma <- qnorm(gamma)
n <- ceiling(1 / (delta^2/(z_gamma^2 * sum(W_h * s2hat_pil)) + 1/N))
...
1 & `r N_h[1]` & `r p_piloto[1]` & `r s2hat_pil[1]` \\
2 & `r N_h[2]` & `r p_piloto[2]` & `r s2hat_pil[2]` \\
\end{tabular}
\end{table}
...
tau_hat_piloto <- mean(tau_amostra_piloto$Q32C)
tau_s2hat_piloto <- var(tau_amostra_piloto$Q32C)
```
...
N = ceiling(1 / ((tau_delta^2/(tau_s2hat_piloto^2*tau_z_t^2)) + 1/N))
```
...
```{r total-inf}
tau_amostra <- sample_n(questionario, N)
tau_f <- N / N

tau_hat <- mean(questionario$Q32C)
tau_s2hat <- var(questionario$Q32C)
tau_varhat <- (1 - tau_f) * tau_s2hat / N
```
...
\hline
`r N` & `r N` & `r tau_hat` & `r format(tau_varhat, digits=4)` & `r tau_s2hat`
\end{tabular}
\end{table}
