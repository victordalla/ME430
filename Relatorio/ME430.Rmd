...
```{r plot, fig.cap = "Gráfico de barras para a pontuação média dos candidatos na primeira fase agrupada por estrato. A pontuação média foi estimada através da amostra piloto. \\label{fig:mu-bar}", fig.height=2.5,fig.width=5, fig.pos='H'}
grafico <- group_by(mu_amostra_piloto, Q14) %>% 
...
grafico %>% ggplot(aes(x = reorder(Q14, Media), y = Media)) + 
  geom_bar(stat = "identity", width = 0.5) +
  xlab("Estrato") + 
  ylab("Pontuação média na primeira fase") +
  ggtitle("Pontuação média por estrato na primeira fase.") +
  ylim(c(0,80))
```
