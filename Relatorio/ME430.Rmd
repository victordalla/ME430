...
Os estratos escolhidos foram as respostas agrupadas da Questão 14 do questionário que cada candidato deveria responder. Essa questão é como segue: "Somando a sua renda com a renda das pessoas que moram com você, quanto é, aproximadamente, a renda familiar mensal? O valor do salário mínimo (SM) é de R$ 724,00". As respostas agrupadas determinam os seguintes estratos: 1 - dados faltantes, 2 - até 5 SM, 3 - entre 5 e 10 SM e 4 - mais que 10 SM. Este estrato foi escolhido conjecturando-se que quanto menor a renda, pior o desempenho no vestibular. A amostra piloto confirmou a conjectura como mostra a Figura \ref{fig:plot}.

 Na Tabela \ref{tab:piloto-media}, observa-se que a pontuação foi maior em estratos de maior renda. A amostra piloto também resultou em uma estimativa de $\hat{s}_{es}^2 = `r mu_s2hat_piloto`$. De posse dessa informação, uma amostragem aleatória simples sem reposição (AASs) com precisão $\delta = `r mu_delta`$ requer $n \geq \Big( \frac{\delta^2}{\hat{s}_{es}^2 z_{\gamma}} + \frac{1}{N} \Big)^{-1}$ unidades amostrais para garantir que $P(|\mu - \hat{\mu}_{es}| \leq \delta) \geq `r mu_gamma`$ \cite{bolfarine2005elementos}. Para os dados dessa amostra, $n = `r mu_n_ASSs`$.
...
\begin{table}[!h]
\centering
\caption{Informações para cada estrato $h$ para estimação de $\mu$, obtidas da amostra piloto: $W_h$ - número de cadidatos no estrato, $\hat{s}_h^2$ - variância no estrato estimada pela amostra piloto, $n_h$ - tamanho amostral do estrato segundo AON.}
...
```{r plot, fig.cap = "Informações sobre a pontuação média dos candidatos na primeira fase separada por estrato. Dados obtidos da amostra piloto. \\label{fig:plot}", fig.height=4,fig.width=6, fig.pos='H'}
grafico <- mu_amostra_piloto %>% group_by(Q14) %>% summarise(Media = mean(TOTAL))

grafico$Q14[grafico$Q14 == 1] <-"Dados faltantes" 
grafico$Q14[grafico$Q14 == 2] <- "Até 5 SM"
grafico$Q14[grafico$Q14 == 3] <- "Entre 5 e 10 SM"
grafico$Q14[grafico$Q14 == 4] <- "Mais que 10 SM"

grafico %>% ggplot(aes(x= Q14, y= Media)) + geom_bar(stat = "identity")+
  xlab("Estratos") + 
  ylab("Pontuação média na primeira fase") +
  ggtitle("Pontuação média na primeira fase calculada por estrato.")+
  ylim(c(0,80))
```
...
A nível populacional, observa-se que existe uma correlação de `r cor_fund1_fund2` entre as respostas das Questões 2 ("Onde você cursou o ensino fundamental 1?") e questão 3 ("Onde você cursou o ensino fundamental 2?"). Assim, conjecturamos que existe uma correlação positiva entre alunos que estudaram completamente em escolas públicas no ensino fundamental 2 e ensino médio. Por isso, para estimar $p$, a proporção de candidatos que estudaram todo o ensino médio em escolas públicas, é proposta a estratificação da população em dois estratos ($H=2$): 1 - candidatos que cursaram o ensino fundamental 1 por completo em escolas públicas e 2 - candidatos restantes.

Sob essa estratificação, foi selecionada uma amostra piloto de tamanho `r prop_n_piloto` sob AE com AP e verificamos a conjectura feita anteriormente. O tamanho final da amostra piloto foi de `r prop_n_piloto_final` devido ao arredondamento feito para $n_h$ (menor inteiro maior que a expressão que determina $n_h$). Essa amostra confirmou a conjectura feita, pois foi obtido uma correlação policórica de `r cor_fund2_ensmed` entre alunos que estudaram completamente em escolas públicas no ensino fundamental 2 e ensino médio. A Tabela \ref{tab:piloto-prop} apresenta outras estatísticas da amostra piloto.
...
Os coeficientes de variação estimados para $\hat{\mu}_{es}$, $\hat{p}$ e $\hat{\tau}$ foram, respectivamente, `r mu_cv`, `r prop_cv` e `r tau_cv`, o que significa que, levando em conta a ordem a magnitude dos dados, o comprimento dos intervalos de confiança gerados por esses estimadores são relativamente pequenos. Além disso, Tabela \ref{tab:comparacao} mostra que para os três parâmetros as estimativas obtidas estão dentro da precisão tomada (ao se escolher o tamanho amostral final), ou seja, o erro absoluto é menor que a precisão usada. Assim, não só os intervalos de confiança obtidos têm comprimentos relativamente pequenos, como contiveram o valor verdadeiro. 
