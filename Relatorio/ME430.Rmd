...
```{r total-piloto}
tau_N <- nrow(questionario)
tau_n_piloto <- 200

tau_amostra_piloto <- sample_n(questionario, tau_n_piloto)
tau_f_piloto <- tau_amostra_piloto/tau_N
...
tau_n = ceiling(1 / ((tau_delta^2/(tau_s2hat_piloto^2*tau_z_t^2)) + 1/tau_N))
```
...
tau_varhat <- (1 - tau_f) * tau_s2hat / tau_n
```
...
\hline
`r tau_N` & `r tau_n` & `r tau_hat` & `r format(tau_varhat, digits=4)` & `r tau_s2hat`
\end{tabular}
\end{table}
