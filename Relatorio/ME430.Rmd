...
\begin{table}[!h]
\centering
\caption{Informações para cada estrato $h$ para estimação de $\mu$, obtidas da amostra piloto: $N_h$ - número de cadidatos no estrato, $\hat{s}_h^2$ - variância no estrato estimada pela amostra piloto, $n_h$ - tamanho amostral do estrato segundo AON para $n = `r mu_n_AE_AP`$.}
...
**Apresentar, em cada seção, as estimativas e fazer referência as tabelas. Esta seção é simples.**
...
# TODO pôr link e slide da fórmula
...
A amostragem final resultou em $\hat{\tau} = `r tau_hat`$ para uma quota inferior do total de quartos nas casas dos candidatos, com variância aproximada de $\hat{Var}(\hat{\tau}_h) = `r tau_varhat`$ e intervalo de confiança de $[`r tau_ic[1]`;`r tau_ic[2]`]$ a um nível de confiança de $95\%$. Essas informações estão sumarizadas na Tabela \ref{tab:inf-tau}.

\begin{table}[!h]
\centering
\caption{Informações da estimação de $\tau$ na amostragem final: $\hat{\tau}$ - total de quartos estimado, $\hat{Var}(\hat{\tau})$ - estimativa da variância do estimador $\hat{\tau}$, $\hat{s}^2$ - variância estimada.}
\label{tab:inf-tau}
\fontsize{11}{13}\selectfont
...
\begin{table}[!h]
\centering
\caption{Comparação das estimativas com valores reais.}
...
$\tau$ & `r tau_hat` & `r tau` & `r tau_delta` & `r erro_abs_tau`\\
\end{tabular}
\end{table}
...
Sob Amostragem Estratificada e Alocação Propocional **QUAL A FÓRMULA NO CASO AASs??? USAR O LIVRO SUGERIDO PELO VITINHO**: 
