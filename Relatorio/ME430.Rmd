...
\begin{table}[!h]
\centering
\caption{Informações para cada estrato $h$ para estimação de $\mu$, obtidas da amostra piloto: $N_h$ - número de cadidatos no estrato, $\hat{s_h^2}$ - variância no estrato estimada pela amostra piloto, $n_h$ - tamanho amostral do estrato segundo AON para $n = `r mu_n`$.}
\label{tab:piloto-media}
\fontsize{11}{13}\selectfont

\begin{tabular}{l|llll}
$h$ & $N_h$ & $\hat{\mu}_h$ & $\hat{\sigma}_h^2$ & $n_h$ \\
\hline
...
\begin{table}[!h]
\centering
\caption{Informações de cada estrato $h$: $\hat{p}_h$ - proporção estimada de canditatos que estudaram o ensino médio completo em escolas públicas, $N_h$ - número de cadidatos no estrato, $\hat{s^2_h}$ - variância no estrato estimada na amostra piloto.}
\label{tab:inf-prop}
\fontsize{11}{13}\selectfont

\begin{tabular}{l|llll}
$h$ & $N_h$ & $\hat{p}_h$ & $\hat{Var(\hat{p_h})}$ & $\hat{s_h^2}$ \\
\hline

1 & `r prop_N_h[1]` & `r prop_phat_h[1]` & `r format(prop_varhat_phat_h[1], digits=4)` & `r prop_s2hat[1]` \\
2 & `r prop_N_h[2]` & `r prop_phat_h[2]` & `r format(prop_varhat_phat_h[2], digits=4)` & `r prop_s2hat[2]` \\
\end{tabular}
\end{table}
